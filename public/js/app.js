...
fetch("/api/whatsapp-link")
  ...
  .then(data => {
    ...
    whatsappBtn.href = whatsappLink; // coloca no botão
  }); */
const STRIPE_PUBLIC_KEY =
  "pk_live_51RLWZiDCgQUWVX4YKX1JwdhjHT8fJ74CXCereV5tdWRFRiwOUzYKJPVJhpouBrtZmOpAxbINCI9QqUaRIpgtCjX600zNEKPFNi";
...
cartBtn.addEventListener("click", function () {
  ...
  if (orderIdElement) {
    orderIdElement.textContent = `Pedido nº: ${currentOrderId}`;
  }
...
menu.addEventListener("click", function (event) {
  if (event.target.classList.contains("size-btn")) {
    ...
    const sizeButtons = event.target
...
function updateCartModal() {
  ...
  if (deliveryFee > 0) {
    ...
    deliveryElement.classList.add("mt-2", "font-semibold");
    deliveryElement.textContent = `Taxa de entrega: R$ ${deliveryFee.toFixed(
      2
    )}`;
...
[nameInput, cpfInput, addressInput, emailInput].forEach((input) => {
  input.addEventListener("input", function (event) {
    if (event.target.value !== "") {
      ...
      document
        .getElementById(`${event.target.id}-warn`)
...
checkoutBtn.addEventListener("click", function () {
  ...
  if (!selectedDelivery) {
    deliveryWarn.classList.remove("hidden");
    deliveryOptions.classList.add("border-2", "border-red-500", "rounded");
    hasError = true;
  }
  if (!hasError) {
    const selectedDelivery = document.querySelector(
    ...
    if (selectedDelivery && selectedDelivery.value === "entrega_fora_campina") {
      const orderData = {
...
// Cálculo conforme opção de entrega
deliveryOptions.addEventListener("change", function (event) {
  const value = event.target.value;

  if (value === "entrega_campina_grande") {
    deliveryFee = 10;
    deliveryLabel = "Entrega em Campina Grande";
  } else if (value === "retirar_loja") {
    deliveryFee = 0;
    deliveryLabel = "Retirada na loja";
  } else {
    deliveryFee = 0;
    deliveryLabel = "Entrega fora de Campina Grande";
  }
...
checkoutStripeBtn.addEventListener("click", async function () {
  ...
  try {
    ...
    saveOrderToFirebase(orderData)
      ...
      .catch((err) => {
        alert(
          "Não foi possível salvar seu pedido no Firebase. Tente novamente."
        );
        console.error(err);
      });
...
checkoutPixBtn.addEventListener("click", function () {
  ...
  checkoutPixBtn.innerHTML =
    '<i class="fa fa-spinner fa-spin"></i> Preparando...';
  ...
  saveOrderToFirebase(orderData)
    ...
    .catch((err) => {
      alert("Não foi possível salvar seu pedido no Firebase. Tente novamente.");
      console.error(err);
    });
...
function sendToWhatsApp(orderData) {
  try {
    const itemsTotal = orderData.cart.reduce(
      (sum, item) => sum + item.price * item.quantity,
      0
    );

    const finalTotal = itemsTotal + orderData.deliveryFee;
...
*DETALHES DO PEDIDO:*\n
${itemsText}\n
*VALOR DOS PRODUTOS:* R$ ${itemsTotal.toFixed(2)}
*FRETE:* R$ ${orderData.deliveryFee.toFixed(2)}
...
*Obrigado por comprar conosco!*`;
    ...
    alert(
      "Pedido concluído! Por favor entre em contato conosco via WhatsApp para finalizar o pagamento via PIX."
    );
  }
}
...
function resetPaymentButtons() {
  ...
  checkoutStripeBtn.innerHTML =
    '<i class="fab fa-cc-stripe"></i> Pagar com Cartão';
  checkoutPixBtn.disabled = false;
  checkoutPixBtn.innerHTML = '<i class="fas fa-barcode"></i> Pagar com PIX';
}
...
window.addEventListener("load", function () {
  ...
  if (urlParams.get("payment") === "canceled") {
    ...
    if (pendingOrder) {
      alert(
        "Pagamento cancelado - seu pedido foi salvo por 1 hora caso queira tentar novamente"
      );
    }
  }
});
...
modal.id = "zoom-modal";
modal.innerHTML = `<img src="" alt="Zoom da imagem">`;
...
function saveOrderToFirebase(orderData) {
  ...
  return firebase.firestore().collection("orders").add(orderData);
}
